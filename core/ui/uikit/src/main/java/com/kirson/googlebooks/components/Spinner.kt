  ...
  Box(modifier = modifier.wrapContentSize(Alignment.TopStart)) {
    ...
    DropdownMenu(
      ...
      onDismissRequest = { expanded = false },
        modifier = dropDownModifier.background(color = GoogleBooksTheme.colors.backgroundSecondary)
    ) {
      options.forEachIndexed { index, element ->
        DropdownMenuItem(
            modifier = Modifier.background(color = GoogleBooksTheme.colors.backgroundSecondary),
            onClick = {
